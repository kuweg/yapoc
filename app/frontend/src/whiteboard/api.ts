export type CardKind = 'note'|'decision'|'question'|'task'|'link'|'artifact'|'actor'|'component'|'service'|'api'|'database'|'queue'|'event'|'interface'|'module'|'boundary'|'external'|'process'|'condition'|'terminator'|'input_output'|'document'|'data_store'|'subprocess'|'manual_input'|'preparation'|'connector'|'delay'
...
export type EdgeColor = 'default'|CardColor
...
export interface BoardEdge { id:string; board_id:string; source_id:string; target_id:string; relationship:Relationship; style:EdgeStyle; direction:EdgeDirection; routing:EdgeRouting; color:EdgeColor; thickness:number; label:string; revision:number; created_by:string; created_at:string; updated_at:string }
...
export const createEdge=(edge:Pick<BoardEdge,'board_id'|'source_id'|'target_id'|'relationship'|'style'|'direction'|'routing'|'color'|'thickness'|'label'>)=>fetch('/api/whiteboard/edges',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(edge)}).then(json<BoardEdge>)
export const updateEdge=(edge:BoardEdge,changes:Partial<BoardEdge>)=>fetch(`/api/whiteboard/edges/${edge.id}`,{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify({...changes,revision:edge.revision})}).then(json<BoardEdge>)
